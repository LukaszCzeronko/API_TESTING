...
public class HttpMethodsTest {
  ...
  public void testHttpMethods(String testCaseNumber, String methodName, int httpCode) {
    Response response = given().when().request(methodName, BASE_URL);
    assertEquals(response.statusCode(), httpCode, testCaseNumber+"Wrong response status code.");
  }
}
